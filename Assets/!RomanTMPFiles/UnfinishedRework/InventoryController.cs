...
namespace UnfinishedRework
{
    public class InventoryController : MonoBehaviour
    {
        ...
        public bool CanInsertMultipleItems(ItemGrid itemGrid, List<Item> itemDatas)
        {
            List<Item> items = new(itemDatas);
            if (items.Count == 0)
            {
                return true;
            }
            items.Sort((x, y) => (y.CellSizeWidth * y.CellSizeHeight).CompareTo(x.CellSizeWidth * x.CellSizeHeight));
            Dictionary<InventoryItem, int> placedInventoryItems = new();
            //^Дикционарий для того, чтобы запомнить, какие вещи были помещены и сколько каждой вещи было помещено.
            //Это нужно, чтобы если не получится поместить все, знать сколько чего и куда было помещено и убрать столько, сколько нужно.
            //(Если некоторые были помещены в стак, то нужно убрать не весь стак, а столько, сколько в него положили, так ведь?)
            ...
            {
                InventoryItem placedItem = TryCreateAndInsertItem(itemGrid, item, 1, 0, true);
                if (placedItem != null)
                {
                    if (placedInventoryItems.ContainsKey(placedItem))
                        placedInventoryItems[placedItem]++;
                    else
                        placedInventoryItems.Add(placedItem, 1);
                }
                else
                {
                    foreach (var dictionaryItem in placedInventoryItems)
