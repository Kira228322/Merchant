    ...
    public class QuestPanel : MonoBehaviour
    {
        ...
        public void Initialize(Quest quest)
        {
            ...
            if (_quest.ExperienceReward != 0) 
            {
                ...
                _rewardTexts[0].text = _quest.ExperienceReward.ToString();
            }
            if (_quest.MoneyReward != 0)
            {
                _rewardTexts[1].gameObject.SetActive(true);
                _rewardTexts[1].text = _quest.MoneyReward.ToString();
            }
            ...
            for (int i = 0; i < _quest.Goals.Count; i++)
            {
                switch (_quest.Goals[i].CurrentState)
                {
                    ...
                    case Goal.State.Completed:
                        _goalTexts[i].text = _quest.Goals[i].Description + ": " + _quest.Goals[i].RequiredAmount + " / " + _quest.Goals[i].RequiredAmount;
