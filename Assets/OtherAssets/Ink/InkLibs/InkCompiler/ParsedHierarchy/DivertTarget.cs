        ...
        public override void ResolveReferences(Story context)
        {
            ...
            while (usageContext && usageContext is Expression)
            {
                ...
                if (usageParent is BinaryExpression)
                {
                    ...
                    if (binaryExprParent.opName is not "==" and not "!=")
                    {
                        badUsage = true;
                    }
                    else
                    {
                        if (binaryExprParent.leftExpression is not (DivertTarget or VariableReference))
                        {
                            badUsage = true;
                        }
                        if (binaryExprParent.rightExpression is not (DivertTarget or VariableReference))
                        {
                            badUsage = true;
                        }
                    }
                ...
                else if (usageParent is Conditional or ConditionalSingleBranch)
                {
                    badUsage = true;
                    foundUsage = true;
                }
